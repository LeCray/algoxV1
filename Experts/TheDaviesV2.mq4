...
/*if(LWMAmovingAvg30M < SMAmovingAvg30M) {
   ...
   if(buying && size_1 && size_2 && size_3 && size_4)
     { //Making sure we have elements to access
      double recentHigh;

      if(minorSwingHighPrices[0]>majorSwingHighPrices[0])
        { //Finding the highest recent price
      ...
      if(Close[0]<recentHigh)
        {
         if(lowCondition_1 || lowCondition_2 || lowCondition_3)
           {   //Making sure we have higher lows

            if(recentHigh>=minorSwingHighPrices[0] && recentHigh>=majorSwingHighPrices[0]) 
              { //Making sure recentHigh is higher than prev minor and major highs
               ...
               if(recentHighBar_1!=0 && recentHighBar_2!=0) 
                 { //Making sure calc is not run on the bar that is currently making the highest high
                  if(minorSwingLowBars[0]>recentHighBar_1 && minorSwingLowBars[0]>recentHighBar_2) 
                    { //Making sure calc is based on a low that is BEHIND the recentHigh bar
                           ...
                           if(High[0]>High[1]) 
                             {
                              ...
                              if(orders==0) 
                                { //Making sure only 1 order is open at a time
